...
export async function fetchProducts({ page, per_page }: GetProductsParams = { page: 1, per_page: 5 }) {
  ...
  try {
    ...
    return {
      data: response.data.data, status: response.status, statusText: response.statusText, pageNum: response.data.page, perPage: response.data.per_page, quantity: response.data.total, totalPages: response.data.total_pages
    };
  } catch (error: any) {
